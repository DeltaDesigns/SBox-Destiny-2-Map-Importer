...
public sealed class DestinyLightShafts : BasePostProcess, BasePostProcess.ExecuteInEditor
{
	...
	[Property, MakeDirty, Range( 0f, 2000000f )]
	public float LightShaftDistance { get; set; } = 500000f;

	#region Shaft Steps
	// radial_blur_8 steps
	private Vector4[] _ultra8Steps = new Vector4[]
	{
		new Vector4( 1f,1f,1f,1f ),
		new Vector4( 1f,1f,1f,1f ),
		new Vector4( 1f,1f,1f,1f ),
		new Vector4( 1f,1f,1f,1f ),
		new Vector4( 0.25f, 0.00f, 0.00f, 0.00f )
	};
	...
	private Vector4[] _ultra12Steps = new Vector4[]
	{
		new Vector4( 1f, 0.94737f, 0.88076f, 0.82063f ),
		new Vector4( 0.7633f, 0.71064f, 0.66264f, 0.61931f ),
		new Vector4( 0.578f, 0.54005f, 0.49546f, 0.41423f ),
		new Vector4( 0.34446f, 0.31711f, 0.29219f, 0.26968f ),
		new Vector4( 0.21432f, 0.00f, 0.00f, 0.00f )
	};
	...
	public void RenderLightShafts()
	{
		...
		Vector2 rtSize2 = Camera.ScreenRect.Size / (LightShaftQuality == LightShaftMode.Cinematic ? 1 : 2);
		Vector2 rtSize4 = Camera.ScreenRect.Size / (LightShaftQuality == LightShaftMode.Cinematic ? 1 : 4);
		Vector2 rtSize8 = Camera.ScreenRect.Size / (LightShaftQuality == LightShaftMode.Cinematic ? 1 : 8);
		//Log.Info( rtSize );
		...
		//Log.Info( $"{screenPosition} : Behind {isBehind}" );

		commandsLightShafts.Attributes.Set( "LightShaftDistance", LightShaftDistance );
		commandsLightShafts.Attributes.Set( "LightShaftDir", new Vector4( screenPosition.x, screenPosition.y, isBehind ? -1f : 1f, 1 ) );
		...
		commandsLightShafts.GlobalAttributes.Set( "DepthZFar", depthZFar.ColorTexture );
	...
	private Vector4[] GetQualitySteps( bool radial8, LightShaftMode mode )
	{
		switch ( mode )
		{
			case LightShaftMode.Cinematic:
				return radial8 ? _ultra8Steps : _ultra12Steps;
			case LightShaftMode.High:
				return radial8 ? _high8Steps : _high12Steps;
			default:
				return radial8 ? _medium8Steps : _medium12Steps;
		}
	}
