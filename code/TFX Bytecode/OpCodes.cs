...
public sealed class TfxBytecodeOp
{
	...
	public static string TfxToString( TfxData tfxData, Vector4[] constants )
	{
		string output = "";
		switch ( tfxData.data )
		{
			...
			case PushConstantVec4Data:
				output = $"constant_index {((PushConstantVec4Data)tfxData.data).constant_index}: Constant value: {constants[((PushConstantVec4Data)tfxData.data).constant_index].ToString()}";
				break;
			case LerpConstantData:
				output = $"constant_start {((LerpConstantData)tfxData.data).constant_start}: Constant 1: {constants[((LerpConstantData)tfxData.data).constant_start]}: Constant 2: {constants[((LerpConstantData)tfxData.data).constant_start + 1]}";
				break;
			case Spline4ConstData:
				output = $"constant_index {((Spline4ConstData)tfxData.data).constant_index}";
				break;
			case Spline8ConstData:
				output = $"constant_index {((Spline8ConstData)tfxData.data).constant_index}";
				break;
			case Spline8ChainConstData:
				output = $"constant_index {((Spline8ChainConstData)tfxData.data).constant_index}";
				break;
...
public struct Spline8ChainConstData
{
	public byte constant_index;
}
